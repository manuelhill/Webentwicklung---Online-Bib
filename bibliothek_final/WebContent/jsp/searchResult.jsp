...
<html lang="de">
	...
	<body>
		...
		{
			out.println("Ihre Suche ergab keinen Treffer");	
		}
		else
		{
		%>
		
		Dies sind Ihre Suchergebnisse<br /><br/>
		<table id="tb_searchResults">
			<tr>
				<th>Buch:</th>
				<th>Link zum Buch:</th>
			</tr>
			<c:forEach var="id" items="${id}" varStatus="counter">
				<tr>
					<td><c:out value="${title[counter.index]}" /></td>
					<td><a href="book.jsp?id=${id}">Link</a></td>
				</tr>
			</c:forEach>
		</table>
		<%} %>
